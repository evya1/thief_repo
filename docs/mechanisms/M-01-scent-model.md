...
The subtractive form is the project's default because it is the arithmetic that `wire_shape: reference-v3` transmits, so a run starts without a negotiation round-trip about scent physics. That selection is an **operational convention** recorded in `docs/decisions/ADR-004-operational-interoperability-profile.md`. It is **not** a lecturer clarification, it is **not** a resolution of OPEN-009, and it must never be presented as the official reading of the source.
...
## B. Supported implementation profiles (operational convention)

Two profiles are implemented behind one common interface. Each is a registered model with a pinned parameter document, so a peer declares which one it computes and a mismatch is detected before play rather than mid-game. The selection is an operational convention recorded in `docs/decisions/ADR-004-operational-interoperability-profile.md`; it is a project decision about what is built, never a course requirement.

### B.1 Default — `subtractive_chebyshev_v1`

The arithmetic that `wire_shape: reference-v3` transmits, and therefore the project default.
...
| Conformance vector | The committed `subtractive_chebyshev_v1` emission, repeated-emission, decay, edge, and corner vectors, plus its registered parameter document and lock hash (T005) |

Worked illustration of the transition, taken from this profile's registered parameter document: a single emission at the center of a 7×7 board yields `0.9` at the center, `0.6` at Chebyshev distance 1, and `0.3` at distance 2; one decay step takes those to `0.8`, `0.5`, and `0.2`.

### B.2 Additional supported — `multiplicative_book_v1`

The verbatim reading of the source's printed figure-4 kernel, kept as the explicit source-oriented alternative. It is **not** deleted or downgraded merely because it is not the default.

| Aspect | Behavior |
|---|---|
| Kernel | The printed 5×5 figure-4 table, looked up verbatim by `(dr, dc)` offset from the emitting cell — never recomputed from a fitted closed form, because two teams fitting their own Gaussian produce different fields |
...
| Clamp policy | `[0.0, 0.9]`. The **upper** clamp is not in the printed formula, which shows only `max(0, ...)`; this profile applies it because the source states intensity is a continuous value in `[0, 0.9]`. Without it, a decayed cell that takes an adjacent deposit exceeds the center intensity |
...
| Conformance vector | The committed `multiplicative_book_v1` kernel, emission, saturation, decay, edge, and corner vectors, plus its registered parameter document and lock hash (T005) |

**Its relationship to the source, stated plainly.** The kernel values are the printed values, and the multiplicative decay and full-turn cadence follow the printed recurrence directly. The upper clamp, the exact evaluation order, and the no-rounding policy are *interpretation* — OPEN-009 resolved in one particular direction so the profile is implementable and comparable. None of it is a lecturer clarification, and selecting this profile would not close OPEN-009 either.
...
- The default selection is `subtractive_chebyshev_v1` (section B.1).
...
- Refusal follows the recorded interoperability convention: a mismatch fires **only when both peers declare the family and the declared values disagree**. One side declaring while the other is silent is not a mismatch and must not refuse.
...
This traces the source's multiplicative decay and nothing else. `multiplicative_book_v1` follows this decay form; `subtractive_chebyshev_v1` does not, and would take that `0.4` to `0.3` rather than `0.36`. Conformance truth lives in each profile's committed vectors, never in this example.
...
- [ ] The source's multiplicative recurrence is demonstrated by `multiplicative_book_v1`, which reproduces the non-saturating worked example above to floating-point tolerance. The default `subtractive_chebyshev_v1` intentionally diverges from that decay form and is validated instead against its own committed vectors and the `{#model_lock}` criteria below. {#scent_recurrence}
...
T005 (`STRAT-002…005, CFG-001, CFG-004`). `{#model_lock}` is not gated on an official OPEN-009 answer: the recorded convention is sufficient to implement, select, and declare a model. OPEN-009 remains officially open and the selected profile is confirmed before counted play.
