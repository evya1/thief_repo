...
| OPEN-003 | RESOLVED TEAM INPUT | Confirmed non-secret metadata: team name `ZeroOne`, team number `01`, group code `ZeroOne0`, GitHub handles `evya1` and `Us5rName`, both role repository URLs, and completed counted opponent `bestteam`. | Resolved for project artifacts, counted play, and final reporting identifiers. | Keep endpoint credentials and transient tunnel values outside tracked artifacts; retain the sanitized completed-match evidence. | orchestrator + project team |
...
| OPEN-010 | RESOLVED HUMAN CONFIRMATION | Final publication metadata is confirmed: team `ZeroOne`, number `01`, handles `evya1` and `Us5rName`, group code `ZeroOne0`, and both repository URLs. | Resolved for counted play and final submission. | Preserve the confirmed public values and keep private identity and credential data outside the repository. | project team |
...
Four named classes group the remaining `OPEN-*`/`INPUT-*` items by *when* they become blockers:

| Gate | Class | Covers | Ready when |
|---|---|---|---|
| `G-OFFICIAL` | official artifact intake | INPUT-001…008; OPEN-001, 002, 004, 005 (label only), 006, 007, 008, 009 | the course supplies the file or the answer |
| `G-PROFILE` | implementation-profile decisions | PLANQ-002…008 | project team decides |
| `G-TEAM` | public team metadata | INPUT-009; OPEN-003 (team part), OPEN-010 | **resolved** — confirmed 2026-08-24 |
| `G-LIVE` | live pairing / opponent / endpoints | INPUT-010; OPEN-003 (remainder) | **resolved** — counted match completed 2026-08-24 |

## Implementation Decision Register

The following items are project decisions, not official requirements and not substitutes for the active OPEN items above. Resolve them during the relevant task-planning step; the project team approves the value. Record only sufficiently important and durable technical decisions in an ADR. Use a Change Request only for a material change to an approved requirement or PRD contract. Additional implementation work receives a new stable task ID instead of silently expanding an active task.
...
| PLANQ-003 | Is an external language-model provider needed, and if so which provider/model, call cadence, token/cost budget, and rate limits? | Optional P2 work only. | **RESOLVED (updated 2026-08-26).** Deterministic template mode remains the default legal path. The optional production path uses OpenRouter through a dependency-free standard-library HTTP client, model `deepseek/deepseek-v4-flash-0731:nitro`, no provider slug so Nitro selects the fastest healthy provider, `OPENROUTER_API_KEY`, optional `OPENROUTER_BASE_URL`, 30-second per-step deadline, 10 output tokens, every eligible step, and the shared Gatekeeper's 30-request-per-minute default with at most one retry. An explicitly configured provider slug remains supported and pins routing with provider fallback disabled. CI uses fake transports and never performs a live call; live evidence remains explicit opt-in. | project team + orchestrator | T013, T017, T027, T050, T051, T060 |
