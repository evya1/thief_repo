...
| OPEN-003 | TEAM INPUT | Confirmed non-secret metadata: team name `ZeroOne`, team number `01`, group code `ZeroOne1`, GitHub handles `evya1` and `Us5rName`, and the two role repository URLs (`https://github.com/evya1/police_repo`, `https://github.com/evya1/thief_repo`). The preserved real-game record identifies the played opponent as `bestteam`; endpoint and tunnel values remain runtime-only private configuration. | Resolved for project artifacts and final reporting identifiers. | Keep endpoint credentials and transient tunnel values outside tracked artifacts; record only sanitized match evidence. | orchestrator + project team |
