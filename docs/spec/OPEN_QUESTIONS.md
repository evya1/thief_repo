...
| PLANQ-005 | Which Gmail sender implementation will satisfy the reporting and security requirements? | Verify send-only `gmail.send`, central Gatekeeper use, exact JSON attachment bytes, idempotency, secret handling, and tests. A draft creator or pretty-printed message-body substitute is noncompliant for counted reporting. | **RESOLVED (2026-08-18) — `implementation_status: RESOLVED_LOCALLY`** (a project-team-owned choice, not an official course requirement and not itself an official Google fact — it selects among options that official Google API mechanics make available). `google-api-python-client` + `google-auth-httplib2` + `google-auth-oauthlib` (structured `dependency_request`, not yet installed). OAuth scope exactly `https://www.googleapis.com/auth/gmail.send` (official Google API fact, per current Google Gmail API documentation). Send via `service.users().messages().send(userId="me", body={"raw": raw_b64}).execute()` (official Google API fact); `drafts().create()` and pretty-printed message-body substitutes are noncompliant (project-owned compliance rule). MIME via stdlib `email.message.EmailMessage`, report bytes as one `application/json` attachment, `base64.urlsafe_b64encode` (official Google API mechanics). `credentials.json`/`token.json` git-ignored, never logged (project-owned security decision). Idempotency is application-owned: a per-`game_id` result-hash sent-state guard, since Gmail's API has no send-side idempotency key (official Google API fact: no idempotency key exists; the guard itself is a project decision). Grounded in current official Google Gmail API Python documentation (see `docs/evidence/c06-prep-01/inputs/google_gmail_api_python.md`); analyzed by deepseek-v4-pro, reviewed and approved by glm-5.2 (`docs/evidence/c06-prep-01/analysis.md`). This resolves the team's implementation choice only — it does not touch `official_status` for OPEN-001, OPEN-004, or OPEN-008, all of which remain OPEN. | project team + orchestrator | T017, T018 |
